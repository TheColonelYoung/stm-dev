...
In order to use freertos-addons initialization and start of scheduler generated by CubeMX must be removed.  
That is same with generated default thread.  
```c++  
osKernelInitialize();  
...
Then in main function are created FRA(freertos-addons) threads and started scheduler.  
```c++  
...
Initialization of USB and handling must be idealy done in same thread after scheduler is started.  
